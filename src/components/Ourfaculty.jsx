...
const FacultyCard = ({ department, imageUrl, altText, color, icon }) => (
  <div className="relative group w-full md:w-1/3 cursor-pointer">
    <img alt={altText} className="w-full h-auto object-cover" height="600" src={imageUrl} width="400" />
    <div 
      className="absolute bottom-6 left-6 right-6 border-4 p-6 flex flex-col gap-3 bg-transparent transition-colors duration-300 group-hover:bg-black/50"
      style={{ borderColor: color }}
    >
      <div style={{ color: color }} className="text-3xl">{icon}</div>
      <h3 className="text-white text-lg font-semibold">{department}</h3>
      <a href="#" style={{ color: color }} className="font-semibold text-sm uppercase tracking-widest flex items-center gap-1">
        LEARN MORE
        <ChevronRightIcon />
      </a>
    </div>
          ...
          <div className="max-w-[1280px] mx-auto flex flex-col md:flex-row items-center md:items-start md:justify-start px-6 md:px-12 py-16 md:py-24 gap-12">
            <div className="flex flex-col justify-center md:justify-start md:items-start gap-8 md:gap-12 w-full md:w-1/4">
              <div className="flex items-center gap-6">
                ...
                <h2 className="text-4xl md:text-5xl font-semibold leading-tight max-w-[160px]">
                  OUR
                  <br />
                  FACULTY
                </h2>
              </div>
              <button className="bg-[#b61f9f] text-white font-semibold text-xs md:text-sm uppercase tracking-widest px-6 py-3 w-max hover:bg-opacity-80 transition-colors" type="button">
                MEET OUR FACULTY
              </button>
            </div>
